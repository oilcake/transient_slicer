...
import os, aifc
import numpy as np
from aubio import source, onset, sink, fvec
...
#     hop_s_out = win_s_out // 2       # hop size
    ...
    with source(filename_given, samplerate, HOP_S_OUT) as s_out:
        ...
        for frames in s_out:
            ...
            balls = 0
            # snk(frames, HOP_S_OUT)
            # np.array(one_note_out.extend(frames))
            # print(type(one_note_out[1]))
...
def file_out(path, file_info, file_name_mask):
    ...
    if os.path.isfile(filename_out):
        with sink(filename_out, 48000, 1) as snk:
            snk(fvec(one_note_out), 4096)
...
for folder in os.listdir(path_out):
    ...
    for sliced_stuff in os.listdir(path_inside):
        ...
        find_biggest_dick_and_stop_point(path_inside + sliced_stuff)
        ...
        if file_info[1] < 8000:
            print('too short')
            continue
        # file_out(path_out + folder, file_info, folder)
